...
DAILY_QUOTES = [
    {
        ...
        "author_en": "Wang Zengqi",
    },
    {
        "quote": "愿你眼中有光，心中有爱，一路春暖花开。",
        "author": "席慕蓉",
        "quote_en": "May your eyes hold light, your heart hold love, and blossoms greet your journey.",
        "author_en": "Hsi Muren",
    },
    {
        ...
        "author_en": "Lin Qingxuan",
    },
    {
        "quote": "山海自有归期，风雨自有相逢，未来皆是坦途。",
        "author": "余光中",
        "quote_en": "Mountains and seas meet in time; the road ahead is wide and bright.",
        "author_en": "Yu Kwang-chung",
    },
    {
...
class ApiModelView(GenericViewSet):
    ...
    def _is_english(self, request):
        ...
        elif not lang and hasattr(request, "META"):
            lang = request.META.get("HTTP_ACCEPT_LANGUAGE", "")
        return "en" in str(lang).lower()

    def _build_date_info(self, now=None, is_en=False):
        if now is None:
        ...
        return {
            ...
            "weekday": WEEKDAYS_EN[weekday_idx] if is_en else WEEKDAYS_ZH[weekday_idx],
            "weekday_en": WEEKDAYS_EN[weekday_idx],
            "weekday_zh": WEEKDAYS_ZH[weekday_idx],
        }

    def _get_daily_quote(self, seed_str=None, is_random=False):
        if is_random or not seed_str:
    ...
    def _format_widget_response(self, wall, now=None, is_random=False, is_en=False):
        if now is None:
            now = timezone.localtime(timezone.now())

        date_str = now.strftime("%Y-%m-%d")
        date_info = self._build_date_info(now, is_en=is_en)
        quote_info = self._get_daily_quote(seed_str=date_str, is_random=is_random)
        ...
        if is_en:
            title = wall.description_en or wall.description or "Daily Featured Art"
            quote = quote_info.get("quote_en", quote_info["quote"])
            quote_author = quote_info.get("author_en", quote_info["author"])
        else:
            title = wall_data.get("title") or wall.description or "每日精选灵感"
        ...
        return {
            "id": wall.id,
            "title": title,
            "description": wall.description_en if is_en else (wall_data.get("description") or quote),
            "quote": quote,
            "quote_author": quote_author,
            "quote_en": quote_info.get("quote_en", ""),
            "author_en": quote_info.get("author_en", ""),
            "picurl": full_picurl,
            "small_picurl": small_picurl,
            "medium_picurl": medium_picurl,
            "score": wall_data.get("score") or 9.5,
            "classify_name": wall_data.get("classify_name") or "Featured",
            "date_info": date_info,
    ...
    def daily(self, request):
        ...
        now = timezone.localtime(timezone.now())
        date_str = now.strftime("%Y-%m-%d")
        is_en = self._is_english(request)
        ...
        if not walls:
            return Response({"error": "No wallpapers available for widget"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response(self._format_widget_response(selected_wall, now, is_random=False, is_en=is_en))

    @action(detail=False, methods=["get"])
    def random(self, request):
        """
        桌面小组件点击“换一张”快捷刷新接口（每次点击换一张壁纸 + 换一条精美日签）
        """
        now = timezone.localtime(timezone.now())
        is_en = self._is_english(request)
        params = getattr(request, "query_params", getattr(request, "GET", {}))
        ...
        return Response(self._format_widget_response(selected_wall, now, is_random=True, is_en=is_en))
