...
def lambda_handler(event, context):
    ...
    if not is_valid_ip(ip):
        return {"statusCode": 400, "body": json.dumps({"error": "Invalid IP address"})}
    ...
    try:
        ...
        return {"statusCode": 500, "body": json.dumps({"error": f"Database query failed: {str(e)}"})}
...
if __name__ == "__main__":
    # 测试代码
    test_event = {"queryStringParameters": {"ip": "89.213.179.150"}}
